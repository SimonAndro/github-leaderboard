...
function github_leaderboard_metabox_forms( $post ) {
	...
	$github_leaderboard_account_img = get_post_meta( $post->ID, 'github_leaderboard_account_img', true );
	$github_leaderboard_account_cover_img = get_post_meta( $post->ID, 'github_leaderboard_account_cover_img', true );
	...
	$github_leaderboard_vote_total_count = (int)get_post_meta($post->ID, 'github_leaderboard_vote_total_count',true);
...
<table class="form-table github_leaderboard_meta_table">
	<tr>
		...
		<td>
			<select class="widefat" id="github_leaderboard_status" name="github_leaderboard_status" value="" required>
				<option value="week"
					<?php if($github_leaderboard_status == 'week') echo esc_attr('selected','github_leaderboard');?>>
					@Week</option>
				<option value="month" disabled
					<?php if($github_leaderboard_status == 'month') echo esc_attr('disabled','github_leaderboard');?>>
					@Month (Premium Only)</option>
				<option value="year" disabled
					<?php if($github_leaderboard_status == 'year') echo esc_attr('disabled','github_leaderboard');?>>
		...
		<td style="float:left;">
			...
			<option value="list"
				<?php if($github_leaderboard_style == 'list') echo esc_attr('selected','github_leaderboard');?>>List
			</option>
			<option value="grid" disabled
				<?php if($github_leaderboard_style == 'grid') echo esc_attr('selected','github_leaderboard');?>>Grid (Premium Only)
			</option>
			</select>
		</td>
	</tr>
	...
	<tr>
		...
		<td>
			<div>Secondary Color<span class="it_epolladmin_pro_badge" style="top: 28px;right: 103px;position: relative;"><i class="dashicons dashicons-star-empty"></i> Premium Only</span></div>
			...
			<table class="widefat">
				<tr>
					<td colspan="4" class="github_leaderboard_pro_alert_col"><a target="_blank"
							href="https://2times180.com/product/ghLeaderboard-pro/">Get Pro Version to Active this and other
							Features &nbsp;&nbsp;
...
<table class="form-table" id="github_leaderboard_append_option_filed">
	...
	foreach($github_leaderboard_account as $github_leaderboard_opt):
			$leaderboardKEYIt = (float)$github_leaderboard_account_id[$i];
			$github_leaderboard_vote_count = (int)get_post_meta($post->ID, 'github_leaderboard_vote_count_'.$leaderboardKEYIt,true);
			
			if(!$github_leaderboard_vote_count){
				$github_leaderboard_vote_count = 0;
			}
	?>
